...
| **market data** (a spread, a total, a line) | capture it in `src/espn_scoreboard`, gate the leagues allowed to *show* it with `MARKET_LINE_LEAGUES`, and render it as a quotation — no accent colour, the book named, our words nowhere near it. It may reach a page and never a score: `editorial` is guarded twice, by an AST check that it never names odds and a behavioural one that its output is unchanged when a line is present. Adding a league here is a product decision, not a config change (decision log 2026-09-02) |
| to **measure** something against the market | record it on `game_outcomes` and read it back with a reporting function. The line must be captured **pregame** — ESPN drops odds the moment a game starts, so `game_outcomes.pregame_lines` recovers it from `schedule_cache` rather than fetching it — and the upsert `COALESCE`s those columns so a nightly re-run reading an empty board cannot erase them. Recording is not consuming: no scorer or editorial module may name these columns, and a test enforces that (decision log 2026-09-05) |
| a **league-wide page** (standings, trending, a race) | the same view/route/template trio, plus a seed in `web/management/commands/export_static._SEEDS` **and** a link in the header menu. Both halves matter: the menu renders on every page, so a link to a page the exporter never built is one dead link *per page* — the NFL archive produced 640 that way. An entry with no page yet is plain text, not an anchor, and a test holds that |
| a metric to **Player Trends** | a section dict in `services/<league>_trending.py`: a `title`, a `subtitle`, a `context` naming both windows, `columns`, and a `display` of `streak` / `comparison` / `tile`. Rows come from that module's `_row()` — one shape for every metric and both leagues, with **every value pre-formatted in the service**, so the template does no arithmetic and no phrasing. A new metric is a declaration; only a genuinely new *shape* is a template change. A test asserts the two leagues have not drifted apart, because one shared template with two producers fails silently — WNBA rendered nothing at all while it sat on the old contract (decision log 2026-09-02) |
| a league to an existing **standings** page | `src/standings_collector.LEAGUES` if it is scheduled from ESPN — its team ids will then match natively. **MLB is the exception** and comes from StatsAPI, because that is what schedules it; pairing the two would mean joining on team names. A points-based league (MLS) keeps its own table and its own columns rather than being forced into W-L-GB |
| a **season schedule** to browse (week / team) | `src/<league>_schedule.py` (store + collector) and enumerate both axes in `_SEEDS` from the collected table, never hard-coded — a season the collector has not reached yet then seeds nothing instead of empty pages. Keep it separate from any *played-games* table: those answer analytical questions in a different id space |
| **when a seasonal page is allowed to exist** | `services/playoff_window.py`. Gate on **games remaining**, not a date — a date breaks on a lockout or a shortened season and has to be re-derived per league. Thresholds differ by sport because leverage per game does (MLB ~85% of the season played, NFL ~65%). Give it an *off* state too: a finished season may persist, but it must be worded as finished |
...
| a **conclusion the Performance page states** (a trust tier, a calibration read, "is there signal?") | `services/model_trust.py` — the judgements, as pure functions over the diagnostics `performance_context` already computes; `components/results_feed.py` renders what it decides. Put it here, not in the renderer: a claim about the model has to be testable without going through HTML, and every one of these is a way the page could flatter the model |
| to **publish** | `python -m scripts.publish_pages` — it exports, audits internal links, deploys, **checks wrangler's exit code**, and then verifies the *live* URL serves the stylesheet hashes just built. Every local check reads `site-dist/`, the thing the build just wrote, so it cannot catch a stale cache or a half-failed upload; both have happened. It brackets the run with `publish_started`/`publish_finished` records in `logs/update_runs.jsonl` so a **hang** — which reports nothing and exits nothing — is visible afterwards as an unmatched start; `python -m scripts.run_status` reads them |
| a **stylesheet or static asset** | edit it and publish — cache-busting versions are the file's own content hash (`web/assets.py`), never hand-typed. A hand-maintained `?v=` once kept a shipped header rewrite invisible for two days |
| to **compare hit rates** anywhere — markets, bands, months, model versions | `services/base_rates.py`, never a shared average. A rate is only meaningful against the base rate of the exact props being counted; comparing to one blended number reversed the true market ranking (see the 2026-08-19 decision entry). Populations must be knowable **before** the event, and resolution comes from `domain.markets.grade` rather than a re-written comparison |
| **any request to ESPN** | never call the host directly — go through `src.espn_http.espn_get` (or `host_candidates` where the client builds its own URLs, as `ncaaf_collector.fetch_json` does). It tries `site.web.api.espn.com` before `site.api.espn.com`, because the old host 403s many non-curl User-Agents and the project only worked by accident; a URL on neither host passes through untouched (decision log 2026-09-13) |
...
| to **write** that read | `python -m scripts.nfl_dossier <espn_event_id>` prints the engine's read, the measured defence ratings, the live injury report, the **roster check** (suffix-insensitive — the check that caught four wrong departures on 2026-09-13), usage, and any `--line` scored against the record. The method for turning a line into a call, the ±8-point pass threshold and the pre-publish checklist are in `content/nfl/README.md`; price arithmetic is `src/odds.py` |
| a **hand-written read for one game** (availability, roster moves, a shape, prop leans) | `content/nfl/<espn_event_id>.toml`, read by `services/nfl_game_notes.py` and rendered by `components/nfl_game.py`. Dated, sourced, never consumed by scoring; a status or direction typo fails the build. Players it marks out or gone leave the spotlights before the pick (decision log 2026-09-09). Its numbered calls go to the **lean ledger** (`services/nfl_leans.py`, `nfl_lean_ledger`, page `/leans/`): recorded on `python -m scripts.nfl_leans record` or the daily run, graded from ESPN's box score (`src/espn_nfl_boxscore.py`) once final. A pre-line read has no number and is never graded |
| **prior-season fallback for ranking** | `src/prior_season_collector.py` (`LEAGUES` config) + `services/prior_season.py`. Add a league only if its rosters persist between seasons — measure it, do not assume it. The blend and its decay live in `editorial._blend`; past `MIN_GAMES` it must contribute exactly zero |
| **season context for a schedule-only league** (what a team was last year, who has left) | `src/<league>_collector.py` + `src/<league>_store.py` for the data, `services/<league>_context.py` to turn it into `editorial.Signal`s. Emit signals, not HTML — `components/editorial.py` then renders it natively and the caveats keep their equal prominence. Refresh on a **staleness check**, not daily: rosters move in weeks, and NCAAF's full pass is ~530 requests (see `services/ncaaf_context.py`) |
...
| a **matchup page for a league with no feed** | `web/simple_game.py` + `web/templates/web/game_simple.html` — records, rank, stakes, the editorial read, and a plain statement of what a schedule cannot show. Gate it **per game** via `deep_dive_available`, never per league: a card stays compact when there is nothing to say, and a link to a page that can only shrug is exactly that nothing |
