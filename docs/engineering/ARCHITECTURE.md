...
| to **publish** | `python -m scripts.publish_pages` — it exports, audits internal links, deploys, **checks wrangler's exit code**, and then verifies the *live* URL serves the stylesheet hashes just built. Every local check reads `site-dist/`, the thing the build just wrote, so it cannot catch a stale cache or a half-failed upload; both have happened |
