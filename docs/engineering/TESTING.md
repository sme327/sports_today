...
## Test suites (330 tests, all passing)
...
| `test_competition_context.py` | Season/phase/week/round/neutral-site on `SlateGame`: one phase vocabulary across ESPN + MLB StatsAPI, unknown stays `None` (never defaulted to "regular"), `context_label` composition and competition-name dedup, `notable_context` suppressing ordinary regular-season games, and cache rows written before these fields existed still deserializing |
